...
You can run the book using Docker without installing any dependencies:

```sh
# 拉取镜像并运行
docker run -p 8080:80 ghcr.io/leptos-rs/book:latest
...
```sh
# 构建镜像
docker build -t leptos-book .

# 运行容器
docker run -p 8080:80 leptos-book
```
...
services:
  leptos-book:
    image: ghcr.io/leptos-rs/book:latest
    ports:
      - "8080:80"
